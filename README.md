...
Two separate images are built on purpose — training and serving have
different dependencies and different lifecycles, so bundling them
into one image would make the serving image needlessly large:

- **`Dockerfile.serve`** — a multi-stage build using `requirements-serve.txt`
  (no training-only dependencies like `kaggle`, `scikit-learn`,
  `matplotlib`). The builder stage installs packages into a venv;
  the final stage copies only that venv plus the app code, keeping
  the image lean.
- **`Dockerfile.train`** — the full `requirements.txt`, for
  reproducible training runs. `data/` and `model/` are mounted as
  volumes rather than baked into the image, since the dataset is
  large and the resulting model needs to persist on the host.
- **`Dockerfile.frontend`** — the Streamlit demo UI.

Run just the API:

```bash
docker build -f Dockerfile.serve -t catsdogs-api:v1 .
docker run -p 8000:8000 -v ./model:/app/model:ro catsdogs-api:v1
```

Or run the full API + demo UI together with Compose:

```bash
docker compose up --build
```

Then open `http://localhost:8501` for the Streamlit demo, or
`http://localhost:8000/docs` to hit the API directly. The two
services talk to each other over Docker's internal network — the
frontend reaches the API at `http://api:8000` (the Compose service
name), not `localhost`, since they're different containers.

To run training inside Docker instead of locally:

```bash
docker build -f Dockerfile.train -t catsdogs-train:v1 .
docker run -v ./data:/workspace/data -v ./model:/workspace/model catsdogs-train:v1
```

**Note:** `requirements-frontend.txt` pins `streamlit==1.40.0` (not an
earlier version) because `st.image`'s `use_container_width` argument,
used in `frontend/app.py`, was only added in Streamlit 1.40.0 — an
...
├── frontend/
│   └── app.py             # Streamlit demo UI (Phase 6)
...
├── requirements-frontend.txt # trimmed dependencies for the Streamlit container (Phase 6)
├── Dockerfile.serve       # multi-stage build for the API image (Phase 6)
├── Dockerfile.train       # training container image (Phase 6)
